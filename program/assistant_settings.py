...
async def join_chat(c: Client, m: Message):
    ...
    try:
        ...
        return await user.send_message(chat_id, "انا جييت 😹")
    except UserAlreadyParticipant:
        return await user.send_message(chat_id, "موجود اعم في اي 🥲")
...
async def leave_chat(c :Client, m: Message):
    ...
    try:
        if chat_id in QUEUE:
            ...
            return await c.send_message(chat_id, "لقد غادر")
        else:
            await user.leave_chat(chat_id)
            return await c.send_message(chat_id, "تم مغداره المجموعه ")
    except UserNotParticipant:
        return await c.send_message(chat_id,"تم المغادره من فتره")
...
async def start_group_call(c: Client, m: Message):
    chat_id = m.chat.id
    msg = await c.send_message(chat_id, "`جاري تشغيل المكالمه..`")
    try:
...
async def stop_group_call(c: Client, m: Message):
    chat_id = m.chat.id
    msg = await c.send_message(chat_id, "`جارياغلاق الدردشة الصوتيه..`")
    try:
        if not (
            group_call := (
            ...
            await msg.edit_text("تم انها الكول من مده")
            return
        await user.send(
            DiscardGroupCall(
                call=group_call
        ...
        await msg.edit_text("تم الانتهاء ")
