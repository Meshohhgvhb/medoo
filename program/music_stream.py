...
async def play_tg_file(c: Client, m: Message, replied: Message = None, link: str = None):
    ...
    if replied.audio or replied.voice:
        if not link:
            suhu = await replied.reply("⚡~ تحميل ملف...")
        else:
            suhu = await m.reply("⚡~ رفع ملف...")
        ...
        if chat_id in QUEUE:
            ...
            try:
                ...
                userid = m.from_user.id
                image = await thumb(thumbnail, title, userid, ctitle)
                await suhu.edit("⚡~ يتم الشغيل الان")
                ...
                await m.reply_text("⚡~ لا يوجد مكالمه في المجموعه .\n\n-› اكتب .فتح كول وحاول مره اخرى !")
            except Exception as e:
                LOGS.info(e)
    else:
        await m.reply_text(
...
@Client.on_message(command(["شغل", f"ت", f"/play", f"تشغيل", f"ش"]) & other_filters)
@check_blacklist()
